...
- **Sidebar** — single component (no variants), 240px wide, hug height. Structure: optional LogoSlot (40×40, border-bottom) + optional section label (text-xs/Medium, text-secondary) + up to 6 SidebarItem instances (ShowItem1–6 boolean). Right border (`color/border`). Background `color/surface-raised`. Component ID: 503:175. Web: props `logo`, `sectionLabel`, `items[]` (label/href/icon/active/disabled), `className`.
- **Alert** — 4 semantic variants (info/success/warning/danger). Surface bg + full border + semantic icon using same `*-surface`, `*-border`, `*-text` tokens as Badge/Toast. Props: `variant` (required), `title`, `description?`, `action?: { label, onClick }` (text button right of content, variant text colour), `onDismiss?` (× icon right of action). Component set ID: 577:242. Figma has Title, Description, Action label as component text properties.
