...
- **Button** — 4 variants, 3 sizes, 5 states (default/hover/pressed/disabled/loading), hug sizing, prototype interactions, Figma + web. Component properties: `Icon#579:0` (TEXT, default 'add'), `Show icon#89:0` (BOOLEAN, default true — wired to LeadingIcon.visible), `Label#250:0` (TEXT, default 'Button'). **Icon nodes use Material Icons Regular** (no fontFamily variable binding) — required for Plugin API instance insertion. Change to Material Symbols Rounded in Figma UI after builds; then apply `icon / sm/md/lg` text style.
- **IconButton** — 4 variants, 3 sizes, 5 states, hug sizing, square, prototype interactions, Figma + web. Icon text property key: `Icon#579:61` (TEXT, default 'add'). **Icon nodes use Material Icons Regular** (same constraint as Button).
...
- **NavBar** — single component (no variants), 800px reference width × 64px height. Structure: LogoSlot (40×40, `color/surface` bg, border, radius) + NavGroup (4 NavButton instances) + Spacer (FILL) + ActionGroup (3 IconButton ghost md instances). Bottom border only (`strokeBottomWeight=1`, `color/border`). Background `color/surface-raised`. ActionGroup icons: AlertsButton=notifications, ProfileButton=account_circle, ThemeButton=dark_mode. Component ID: 481:166.
- **Breadcrumb** — single component (no variants), hug sizing. Items array with `label` + optional `href`. Last item is current page (no link, `aria-current=page`). Separator: `chevron_right` icon (16px, text-secondary). Figma node: 498:167. Web: `Breadcrumb.tsx`.
- **PaginationButton** — sub-component used inside Pagination. 4 states (default/hover/active/disabled), 32×32px fixed. `Label` text component property. Active: brand bg + text-inverse. Prototype reactions: default→hover, hover→active. Component set ID: 534:207. Nav button instances (prev/next) override Label with Material Icons chevron — fontFamily binding cleared, fontName set to Material Icons Regular directly (Plugin API constraint).
- **Pagination** — composed component. Shows [←][1][…][3][4][5][…][10][→]. `ShowLeftEllipsis` + `ShowRightEllipsis` boolean properties. Ellipsis slots are non-interactive frames. Component ID: 537:225. Web: `page`, `totalPages`, `onPageChange`, `siblingCount` props.
- **SidebarItem** — 4 states (default/hover/active/disabled), full-width (FILL horizontal), hug height. Icon (20px) + label. Active: brand-subtle bg, brand text, Medium weight. Component set ID: 502:187.
- **Sidebar** — single component (no variants), 240px wide, hug height. Structure: optional LogoSlot (40×40, border-bottom) + optional section label (text-xs/Medium, text-secondary) + up to 6 SidebarItem instances (ShowItem1–6 boolean) + Footer (3 IconButton ghost md instances: AlertsButton=notifications, ProfileButton=person, ThemeButton=dark_mode). Right border (`color/border`). Background `color/surface-raised`. Component ID: 503:175. Web: props `logo`, `sectionLabel`, `items[]` (label/href/icon/active/disabled), `className`.
- **Alert** — 4 semantic variants (info/success/warning/danger). Surface bg + full border + semantic icon using same `*-surface`, `*-border`, `*-text` tokens as Badge/Toast. Props: `variant` (required), `title`, `description?`, `action?: { label, onClick }` (Button ghost sm instance, right of content), `onDismiss?` (IconButton ghost sm instance, icon=close, rightmost). Component set ID: 577:242. Figma: Action is a Button ghost sm instance; Dismiss is an IconButton ghost sm instance with icon=close.
- **Badge** — 2 strengths (subtle/strong) × 5 semantic variants (info/success/warning/danger/neutral) = 10 variants. Subtle: tinted surface bg + matching border. Strong: bold solid bg + text-inverse. Pill radius (`Radius/full`). Component set ID: 548:226. Web: `variant` + `strength` props (strength defaults to 'subtle'). Tokens: `color/{variant}-surface`, `color/{variant}-border`, `color/{variant}-text` (dedicated per-variant text token ensuring WCAG AA 4.5:1 on its surface — info: sky-700/sky-200, success: green-800/green-200, warning: amber-800/amber-200, danger: red-700/red-200; neutral uses `color/neutral` which already passes), `color/warning-strong` (amber-700 for WCAG AA with white), `color/neutral-strong` (slate-600 light / slate-500 dark). Label is a component text property in Figma.

### Material Symbols Rounded — Plugin API constraint

**Do NOT call `setBoundVariable('fontFamily', fontIconsVar)` on icon text nodes inside components that will be used as sub-components.** When this binding is applied, Figma sets `fontName.family` to "Material Symbols Rounded". Any `appendChild`/`insertChild`/`clone` of a node containing MSR then fails with "unloaded font Material Symbols Rounded Regular" — MSR is a variable font loaded at file level, not available via `loadFontAsync`. Setting `textStyleId` to an MSR-bound text style also fails for the same reason. `node.remove()` on an MSR node is safe (no font load required).

**Rule (Plugin API builds only):** Use `fontName = { family: 'Material Icons', style: 'Regular' }` as the base font — no fontFamily variable binding. Change to Material Symbols Rounded afterwards in Figma UI.

**Icon text styles:** Three named text styles exist — `icon / sm` (16px), `icon / md` (20px), `icon / lg` (24px). All tokens bound: `fontFamily → Font/icons`, `fontWeight → Font/weight/regular`, `fontSize + lineHeight → Size/16/20/24`. Apply these styles to icon nodes in the Figma UI after switching the font to Material Symbols Rounded.

**Replacing MSR icon nodes via Plugin API:** The only workaround is `node.remove()` (safe) + `figma.createText()` with Material Icons Regular. This was used to fix all 108 Button/IconButton LeadingIcon nodes so those components could be inserted as instances into Alert, NavBar, and Sidebar.

**Web:** The `Icon` component uses `className="material-symbols-rounded"`. No web changes needed when updating icon font in Figma.
